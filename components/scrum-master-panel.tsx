...
import { revealVotes, startVoting, nextStory, addStoryToQueue } from "@/app/actions/room-actions"
...
export function ScrumMasterPanel({ room, onUpdate }: ScrumMasterPanelProps) {
  ...
  const handleStartVoting = async () => {
    try {
      ...
      await startVoting(room.code, room.currentStory || undefined, timerSeconds)
      onUpdate()
    } catch (error) {
    ...
    <Card className="border-2 border-yellow-500 bg-yellow-50 dark:bg-yellow-950">
      ...
      <CardContent className="space-y-4">
        ...
        {!room.votingActive && !room.votesRevealed && (
          <div className="p-3 bg-white dark:bg-gray-800 rounded-lg">
            <Label htmlFor="timer" className="text-sm font-medium text-gray-700 dark:text-gray-300">
        ...
        <div className="grid grid-cols-2 gap-3">
          {!room.votingActive && !room.votesRevealed && (
            <>
              <Button onClick={handleStartVoting} className="w-full" size="lg" disabled={!room.currentStory}>
              ...
              <Dialog open={isAddingStory} onOpenChange={setIsAddingStory}>
                ...
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>Add Story to Queue</DialogTitle>
                  </DialogHeader>
          ...
          {room.votingActive && (
            <Button onClick={handleRevealVotes} className="w-full col-span-2" size="lg" variant="default">
          ...
          {room.votesRevealed && (
            <>
              ...
              <Button onClick={handleStartVoting} variant="outline" className="w-full bg-transparent" size="lg">
        ...
        {!room.currentStory && room.storyQueue.length === 0 && (
          <div className="p-4 bg-blue-50 dark:bg-blue-950 rounded-lg border border-blue-200 dark:border-blue-800">
            ...
            <Button onClick={() => setIsAddingStory(true)} variant="outline" size="sm" className="w-full">
              <Plus className="w-4 h-4 mr-2" />
              Add First Story
            </Button>
          </div>
        ...
        {!room.currentStory && room.storyQueue.length > 0 && (
          <div className="p-4 bg-green-50 dark:bg-green-950 rounded-lg border border-green-200 dark:border-green-800">
            <p className="text-sm text-gray-700 dark:text-gray-300 mb-2">
              Ready to start! Click "Start Voting" to begin with the first story in the queue.
            </p>
          </div>
        )}
      </CardContent>
